...
import {HighlightFileModel} from './dbModel';
...
export default class ImageEditService {
    ...
    static saveHighlightToDb(s3url, email) {
        return new Promise((resolve, reject) => {
            let highlightFileInstance = new HighlightFileModel({username: email, s3url: s3url});
...
function drawText(_filePath) {
    ...
    let outputPath = rootDir + '/' + Date.now() + path.basename(_filePath, '.mp4') + '.mp4';
    ...
    return new Promise((resolve, reject) => {
        let textFilter = {
            filter: 'drawtext',
            options: {
                fontfile: rootDir + '/assets/fonts/arial.ttf',
